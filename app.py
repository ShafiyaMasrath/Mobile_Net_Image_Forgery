...
def load_model(weights_path):
    ...
    if weights_path and os.path.exists(weights_path):
        print(f"✓ Found weights file, loading trained model...")
        model = MobForgeNet(pretrained=False).to(DEVICE)
        state_dict = torch.load(weights_path, map_location=DEVICE)
        model.load_state_dict(state_dict)
        print(f"✓ Model loaded successfully from: {weights_path}")
    else:
        # Fallback: try default checkpoint location
        default_path = os.path.join(os.path.dirname(__file__), 'checkpoints', 'best_model.pth')
        print(f"⚠ Weights not found at {weights_path}, trying default: {default_path}")
        
        if os.path.exists(default_path):
            print(f"✓ Found default checkpoint, loading...")
            model = MobForgeNet(pretrained=False).to(DEVICE)
            state_dict = torch.load(default_path, map_location=DEVICE)
            model.load_state_dict(state_dict)
            print(f"✓ Model loaded successfully from: {default_path}")
        else:
            raise FileNotFoundError(
                f"❌ No trained model found!\n"
                f"   Expected at: {weights_path}\n"
                f"   Or at: {default_path}\n"
                f"   Please train the model first or provide correct weights path."
            )
